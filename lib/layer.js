...
class Layer {
  constructor(setting, NeuronType) {
    if (NeuronType === undefined) NeuronType = Neuron;
    ...
    } else if (setting && setting.constructor === Array) {
      this.neurons = setting;
    } else {
      throw new ValueError('Layer constructor only takes an integer argument or a neuron array.');
    }
  }
   ...
   * @return {Layer}    this
   */
  project(layer, projection) {
    if (projection === undefined) projection = Layer.projection.ALL_TO_ALL

...
class Network {
  ...
  propagate(learningRate, outputs, momentum) {
    this.validate();
    // console.log(this.id, 'propagate', learningRate, outputs, momentum);
    momentum = momentum || 0;
  ...
  validate() {
    if (!(this.input instanceof Layer) || !(this.output instanceof Layer))
      throw new TypeError('Input layer, or output layer, or both are not Layer instances');
  }
...
class MasterNetwork extends Network {
  constructor(network) {
    super();
  ...
  propagate(learningRate, outputs, momentum) {
    ...
    for (let i = stack.length - 1; i >= 0; i--) {
      // console.log(stack.length - 1 - i, outputs.length, stack.length - 1 - i < outputs.length, outputs);
  ...
  findLocalCallStack(network) {
    let stack = [];
    this.visited[network.id] = true;
    for (let i = 0; i < network.previous.length; i++) {
      if (!this.visited[network.previous[i].id]) {
        stack = stack.concat(this.findLocalCallStack(network.previous[i]));
      }
    }
    stack.push(network);
    return stack;

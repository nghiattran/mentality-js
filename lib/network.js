...
class Network {
  ...
  ready() {
    ...
    this.connections = [];
    // Ignore the first connection because it is bias
   ...
   */
  activate(inputs) {
    if (this.input.length !== inputs.length) {
      throw new ValueError('Number of inputs and input neurons don\'t match.');
    }
  ...
  findCallStack() {
    ...
    for (let i = 0; i < this.output.length; i++) {
      this.findLocalCallStack(this.output[i], stack, visited);
    }

    return stack;
  }

  findLocalCallStack(neuron, stack, visited) {
    if (neuron === Neuron.biasNeuron) return;

    visited[neuron.id] = true;
    for (let i = 0; i < neuron.previous.length; i++) {
      if (!visited[neuron.previous[i].from.id]) {
        this.findLocalCallStack(neuron.previous[i].from, stack, visited);
      }
    }
    stack.push(neuron);
    return stack;
